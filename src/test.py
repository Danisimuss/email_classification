...
#Cоздаёт классификатор с ключевыми словами из json
    ...
    clf = Classifier()
    clf.load_keywords("keywords.json")  
    return clf
...
@pytest.mark.parametrize('text', ['', '   ', 'абракадабра без ключевых слов']) #!!!!! менять если путой - черновик
...
def test_empty_file(classifier, temp_file):
    path = temp_file('')
    assert classifier.handle_mail(path)=='Несортированное' #!!!!! менять если путой - черновик
